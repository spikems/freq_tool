...
class W2V:
    ...
    def train(self, size=300, min_count=5, sg=1, iter=20, workers=32):
        ...
        self.w2v = Word2Vec(size=size, min_count=min_count, window=10, alpha=0.05, sg=sg, iter=iter, workers=workers)
        ...
        self.w2v.train(_data, total_words = token_count, epochs = self.w2v.iter)

    def save(self, fname):
        self.w2v.wv.save_word2vec_format(fname, binary=True)
    ...
    def print_model(self, modelfile):
        ...
        if os.path.exists(modelfile):
            word2vec = KeyedVectors.load_word2vec_format(modelfile, binary=True)
