...
def normalize(word):
    return word if word not in normdict else normdict[word]
...
load_normdict(os.path.join(cur_path, u"userdict/normalize.dict"))
...
def norm_cut(text, newwords=None, HMM=True):    # 切词，对应jieba.cut,但是能识别出special_newwords在中的特殊词汇
    """
    ...
    text_cut = jieba.cut(text, HMM=HMM)
    if found_terms is None or found_terms == {}:
        ...
        index = 0
        for word in jieba.cut(current_text, HMM=HMM):
...
#replace ' ' with '-' for special words
def output_special(text):
    return text.replace(' ','-')

def norm_seg(text, newwords=None, new_flag="nz", HMM=True):    # 带词性切词，对应jieba.posseg.cut,但是能识别出special_newwords在中的特殊词汇,同时，这些特殊新词的词性默认为nz
    """
    ...
    if isinstance(newwords,tuple) or isinstance(newwords,set) or isinstance(newwords,list):
        ...
        found_terms=norm_cut_find(newwords=special_newwords,text=text)
    if found_terms is None or found_terms == {}:
        for item in jieba.posseg.cut(text, HMM=HMM):
            yield item
    else:
        current_text = text
        sorted_found_index = sorted(found_terms.keys())
        ...
        psg_list = list(jieba.posseg.cut(current_text, HMM=HMM))
        index = 0
        for item in psg_list:
            ...
            if index not in found_terms:
                ...
                yield pair(output_special(found_terms[index]), new_flag)
                index += len(found_terms[index])
